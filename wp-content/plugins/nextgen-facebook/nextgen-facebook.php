...
if ( ! class_exists( 'Ngfb' ) ) {
	...
	class Ngfb {
		...
		/**
		 * Runs at init priority 11 by default. Called by activate_plugin() as well.
		 */
		...
		/**
		 * Runs at init priority 12 by default. Called by activate_plugin() as well.
		 */
		public function set_objects( $activate = false ) {
			...
			if ( class_exists( 'Wpsso' ) ) {	// just in case.
				...
				$error_msg = __( '%1$s is a newer fork / child of %2$s and the two plugins cannot be active at the same time. <a href="%3$s">Please deactivate and uninstall the %2$s plugin</a>.', 'nextgen-facebook' );
				...
				if ( $activate ) {	// wpsso found on plugin activation
					...
					wp_die( '<p>' . sprintf( $die_msg, $fork_name, $info['name'] ) . '</p>' );
					...
					if ( is_admin() ) {
						$deactivate_url = wp_nonce_url( add_query_arg( array(
							...
							'paged' => 1,
							's' => 'deactivate-plugin_' . $info['base'],
						), admin_url( 'plugins.php' ) ) );

						$this->notice->err( sprintf( $error_msg, $fork_name, $info['name'], $deactivate_url ) );
					}
