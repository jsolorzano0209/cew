...
if ( ! class_exists( 'NgfbPost' ) ) {
	...
	class NgfbPost extends NgfbMeta {
		...
		public function check_post_head_duplicates( $post_id = true, $post_obj = false ) {
			...
			$warning_time = (int) SucomUtil::get_const( 'NGFB_DUPE_CHECK_WARNING_TIME', 2.5 );
			$timeout_time = (int) SucomUtil::get_const( 'NGFB_DUPE_CHECK_TIMEOUT_TIME', 3.0 );
			...
			if ( true === $url_time ) {
				...
				if ( $is_admin && $url_time > $warning_time ) {
					$this->p->notice->warn(
						sprintf( __( 'Retrieving the HTML document for %1$s took %2$s seconds.', 'nextgen-facebook' )
						...
						sprintf( __( 'This exceeds the recommended limit of %1$s seconds (crawlers often time-out after %2$s seconds).',
							'nextgen-facebook' ), $warning_time, $timeout_time ).' '.
						__( 'Please consider improving the speed of your site.', 'nextgen-facebook' ).' '.
						__( 'As an added benefit, a faster site will also improve ranking in search results.', 'nextgen-facebook' ).' ;-)'
					);
				}
			}
