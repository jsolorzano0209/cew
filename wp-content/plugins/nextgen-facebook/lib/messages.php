				...
				if ( strpos( $idx, 'tooltip-meta-' ) === 0 ) {
					switch ( $idx ) {
						...
						case 'tooltip-meta-og_vid_desc':
							...
							$text = sprintf( __( 'The %1$s video API modules can offer a default video name / title and description, when that information is provided by the video API service (not all API services offer detailed information about their videos).', 'nextgen-facebook' ), $info['name_pro'] );
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-site_name_alt':
							...
							$text = __( 'An optional alternate name for your WebSite that you want Google to consider.', 'nextgen-facebook' );
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-plugin_filter_title':
							...
							$text = __( 'The title values provided by WordPress may include modifications by themes and/or SEO plugins (appending the site name, for example, is a common practice). Uncheck this option to always use the original unmodified title value (default is unchecked).', 'nextgen-facebook' ).' '.sprintf( __( 'Advanced users can also hook the \'%s\' filter and return true / false to enable / disable this feature.', 'nextgen-facebook' ), $this->p->lca.'_filter_title' );

							break;

						case 'tooltip-plugin_filter_content':

							$text = __( 'Apply the WordPress \'the_content\' filter to the content text (default is unchecked). The content filter renders all registered shortcodes, which may be required to detect images and videos added by these shortcodes.', 'nextgen-facebook' ).' '.__( 'Some themes / plugins have badly coded content filters, so this option is disabled by default.', 'nextgen-facebook' ).' '.__( 'If you use shortcodes in your content text, this option should be enabled &mdash; if you experience display issues after enabling this option, determine which theme / plugin is filtering the content incorrectly and report the problem to its author(s).', 'nextgen-facebook' ).' '.sprintf( __( 'Advanced users can also hook the \'%s\' filter and return true / false to enable / disable this feature.', 'nextgen-facebook' ), $this->p->lca.'_filter_content' );

							break;

						case 'tooltip-plugin_filter_excerpt':

							$text = __( 'Apply the WordPress \'get_the_excerpt\' filter to the excerpt text (default is unchecked). Enable this option if you use shortcodes in your excerpts, for example.', 'nextgen-facebook' ).' '.sprintf( __( 'Advanced users can also hook the \'%s\' filter and return true / false to enable / disable this feature.', 'nextgen-facebook' ), $this->p->lca.'_filter_excerpt' );
						...
						case 'tooltip-plugin_shortcodes':
							...
							$text = sprintf( __( 'Enable the %s shortcode features (default is checked).', 'nextgen-facebook' ), $info['short'] );

							break;

						case 'tooltip-plugin_widgets':

							$text = sprintf( __( 'Enable the %s widget features (default is checked).', 'nextgen-facebook' ), $info['short'] );

							break;

						case 'tooltip-plugin_page_excerpt':

							$text = __( 'Enable the excerpt metabox for Pages.', 'nextgen-facebook' ).' '.sprintf( __( 'An excerpts is an optional hand-crafted summary of your content, that %s can also use as a default description value.', 'nextgen-facebook' ), $info['short'] );

							break;

						case 'tooltip-plugin_page_tags':

							$text = __( 'Enable the WordPress tags metabox for Pages.', 'nextgen-facebook' ).' '.__( 'WordPress tags are optional keywords about the content subject, often used for searches and "tag clouds".', 'nextgen-facebook' ).' '.sprintf( __( '%s converts WordPress tags into hashtags for some social sites (Twitter, Facebook, Google+, etc.).', 'nextgen-facebook' ), $info['short'] );
						...
						case 'tooltip-plugin_shortener':
							...
							$text = sprintf( __( 'A preferred URL shortening service for %s plugin filters and/or add-ons that may need to shorten URLs &mdash; don\'t forget to define the service API keys for the URL shortening service of your choice.', 'nextgen-facebook' ), $info['short'] );
				 ...
				 */
					...
					switch ( $idx ) {
						...
						case 'tooltip-schema_knowledge_graph':
							...
							$text = __( 'Include WebSite, Organization, and/or Person Schema markup in the front page for Google\'s <em>Knowledge Graph</em>.', 'nextgen-facebook' ).' '.__( 'The WebSite markup includes the site name, alternate site name, site URL and search query URL.', 'nextgen-facebook' ).' '.sprintf( __( 'Developers can hook the \'%s\' filter to modify the site search URL (or disable its addition by returning false).', 'nextgen-facebook' ), $this->p->lca.'_json_ld_search_url' ).' '.sprintf( __( 'The Organization markup includes all URLs entered on the %s settings page.', 'nextgen-facebook' ), $settings_page_link ).' '.__( 'The Person markup includes all contact method URLs entered in the user\'s WordPress profile page.', 'nextgen-facebook' );
							break;

						case 'tooltip-schema_home_person_id':
							$text = __( 'Select a site owner for the (optional) <em>Knowledge Graph</em> Person markup included in the front page.', 'nextgen-facebook' ).' '.__( 'The Person markup includes all contact method URLs entered in the user\'s WordPress profile page.', 'nextgen-facebook' ).' '.sprintf( __( 'The available Person list includes users with \'%1$s\' and/or \'%2$s\' roles.', 'nextgen-facebook' ), _x( 'Administrator', 'User role' ), _x( 'Editor', 'User role' ) );
							break;
						...
						if ( $this->p->check->aop( $lca, false ) ) {
							$text = '<p class="pro-feature-msg"><a href="'.$url['purchase'].'">'.
								sprintf( __( 'Purchase %s licence(s) to install its Pro modules and use the following features / options.',
									'nextgen-facebook' ), $info['short_pro'] ).'</a></p>';
						} else {
							$text = '<p class="pro-feature-msg"><a href="'.$url['purchase'].'">'.
			 ...
			 */
				...
				switch ( $idx ) {
					...
					case 'notice-content-filters-disabled':
						...
						$text = '<p><b>'.sprintf( __( 'The %1$s advanced option is currently disabled.', 'nextgen-facebook' ), $filters_option_link ).'</b> '.sprintf( __( 'The use of WordPress content filters allows %s to fully render your content text for meta tag descriptions and detect additional images / embedded videos provided by shortcodes.', 'nextgen-facebook' ), $info['name'] ).'</p><p><b>'.__( 'Some themes / plugins have badly coded content filters, so this option is disabled by default.', 'nextgen-facebook' ).'</b> '.sprintf( __( '<a href="%s">If you use shortcodes in your content text, this option should be enabled</a> &mdash; if you experience display issues after enabling this option, determine which theme / plugin is filtering the content incorrectly and report the problem to its author(s).', 'nextgen-facebook' ), $settings_page_url ).' '.sprintf( __( 'Advanced users can also hook the \'%s\' filter and return true / false to enable / disable this feature.', 'nextgen-facebook' ), $this->p->lca.'_filter_content' ).'</p>';
					...
					case 'notice-pro-tid-missing':
						...
						if ( ! is_multisite() ) {
							$settings_page_link = $this->p->util->get_admin_url( 'licenses',
								_x( 'Add-on Plugins and Pro Licenses', 'lib file description', 'nextgen-facebook' ) );
					...
					case 'notice-um-activate-add-on':
						...
						$text = '<p><b>'.sprintf( __( 'At least one Authentication ID has been entered on the %1$s settings page,<br/>but the %2$s add-on is not active.', 'nextgen-facebook' ), $settings_page_link, $um_info['name'] ).'</b> '.sprintf( __( 'This Free add-on is required to update and enable the %1$s plugin.', 'nextgen-facebook' ), $info['name_pro'] ).'</p><p>';

						if ( $idx === 'notice-um-add-on-required' ) {
							$text .= '<b>'.sprintf( __( 'Install and activate the %1$s add-on from the %2$s settings page.', 'nextgen-facebook' ), $um_info['name'], $settings_page_link ).'</b>';
						} else {
							$text .= '<b>'.sprintf( __( 'The %1$s add-on can be activated from the WordPress %2$s page.', 'nextgen-facebook' ), $um_info['name'], $plugins_page_link ).'</b> '.__( 'Please activate this Free add-on now.', 'nextgen-facebook' );
						}
