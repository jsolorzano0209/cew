...
if ( ! class_exists( 'NgfbUtil' ) && class_exists( 'SucomUtil' ) ) {
	...
	class NgfbUtil extends SucomUtil {
		...
		public function get_all_times() {
			...
			return false === $has_changed ? $all_times : get_option( NGFB_TS_NAME, array() );
		}
		...
		public function safe_apply_filters( array $args, array $mod, $max_time = 0 ) {
			...
			if ( $max_time > 0 && $total_time > $max_time ) {
				...
				$error_msg = sprintf( __( 'Slow filter hook(s) detected - WordPress took %1$0.3f secs to execute the "%2$s" filter (longer than recommended max of %3$0.3f secs).', 'nextgen-facebook' ), $total_time, $filter_name, $max_time );
				...
				if ( $this->p->notice->is_admin_pre_notices() ) {	// skip if notices already shown
					...
					if ( $is_wp_filter ) {
						...
						$this->p->notice->warn( sprintf( __( 'Slow filter hook(s) detected &mdash; the WordPress %1$s filter took %2$0.3f seconds to execute. This is longer than the recommended maximum of %3$0.3f seconds and may affect page load time. Please consider reviewing 3rd party plugin and theme functions hooked into the WordPress %1$s filter for slow and/or sub-optimal PHP code.', 'nextgen-facebook' ), $filter_api_link, $total_time, $max_time ) . ' ' . sprintf( __( 'Activating the %1$s plugin and clearing the %2$s cache (to re-apply the filter) may provide more information on the specific hook(s) or PHP code affecting performance.', 'nextgen-facebook' ), $query_monitor_link, $info['short'] ), true, $dismiss_key, WEEK_IN_SECONDS );
					} else {
		...
		public function do_metabox_tabs( $metabox_id = '', $tabs = array(), $table_rows = array(), $args = array() ) {
			$this->do_metabox_tabbed( $metabox_id, $tabs, $table_rows, $args );
		}
		...
		public function do_metabox_tabbed( $metabox_id = '', $tabs = array(), $table_rows = array(), $args = array() ) {
			...
			foreach ( $tabs as $tab => $title ) {
				$class_href_key = $class_tabset.$metabox_id.'-tab_'.$tab;
				$this->do_metabox_table( $table_rows[$tab], $class_href_key, ( empty( $metabox_id ) ?
		...
		public function do_table_rows( $table_rows, $class_href_key = '', $class_tabset_mb = '', $class_tabset = '' ) {
			$this->do_metabox_table( $table_rows, $class_href_key, $class_tabset_mb, $class_tabset );
		}

		public function do_metabox_table( $table_rows, $class_href_key = '', $class_tabset_mb = '', $class_tabset = '' ) {
		...
		public function get_ext_req_msg( $ext ) {
			...
			$req_msg .= sprintf( _x( '%s ext. required', 'option comment', 'nextgen-facebook' ), $ext_short );
