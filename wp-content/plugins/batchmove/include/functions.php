...
function execute_action(batchMove $bm, $apost=array()){
	...
	if (!current_user_can(USERLEVEL)) {
		...
		if ( !empty($apost) && $do ) {
			...
			} elseif (isset($apost['submit'])&&$apost['submit'] == 'send-tag') {
...
function show_bm_selector(batchMove $bm){
	...
	$html .= '</td>';
	$html .= '<td>';
    $allCats = wp_dropdown_categories('show_option_all=' . __('all','batch-move'). '&hide_empty=0&hierarchical=1&echo=0&selected=' .
	...
	$html .= '</td>';
	$html .= '<td>';
    isset($_REQUEST['iDate'])||$_REQUEST['iDate']='';
	...
	$html .= '</td>';
	$html .= '<td>';
    isset($_REQUEST['oDate'])||$_REQUEST['oDate']='';
	...
	$html .= '</td>';
	$html .= '<td><button class="button-primary" type="submit" value="Verzend" >'.$bm->frmlabels['send'].'</button>';
	$html .= '</td>';
	$html .= '</tr>';
...
function show_bm_actions(batchMove $bm){
	...
	foreach ($bm->action as $key => $value) {
		$selected = (isset($bm->get['actions'])&& $bm->get['actions'] == $key ) 
                            ? ' selected="selected"' : '';
		$html .=  '<option value="'.$key.'"'.$selected .'>'.$value.'&nbsp;</option>\n';
	}
	$html .= '</select>';
...
function get_a_query(batchMove $bm){
	...
	*/
	...
	//$cat = (isset($gets['qcat']))?$gets['qcat']:isset($gets['cat'])?$gets['cat']:'';
	if ($cat) {
		$qa['cat'] = intval($cat);
	}
...
function get_information (batchMove $bm, $founded=0){
    ...
    $order = (isset($bm->orderbydef[$bm->orderby]))?
                $bm->orderbydef[$bm->orderby]:$bm->orderbydef['date'];
