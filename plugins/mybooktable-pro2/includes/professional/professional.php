...
function mbtpro_universal_buybutton_init() {
	add_filter('mbt_stores', 'mbtpro_add_universal_buybutton');
	add_action('wp_head', 'mbtpro_add_universal_buybutton_css');
	add_filter('mbt_buybutton_editor', 'mbtpro_universal_buybutton_editor', 10, 4);
	add_filter('mbt_format_buybutton', 'mbtpro_universal_buybutton_button', 10, 3);
}
add_action('mbtpro_init', 'mbtpro_universal_buybutton_init');

function mbtpro_add_universal_buybutton($stores) {
	$stores['universal'] = array('name' => 'Universal Buy Button');
	return $stores;
}

function mbtpro_universal_buybutton_editor($output, $data, $id, $store) {
...
function mbtpro_universal_buybutton_button($output, $data, $store) {
	if($data['store'] == 'universal') {
		if(empty($data['url'])) {
			$output = '';
		} else {
...
function mbtpro_add_universal_buybutton_css() {
...
function mbtpro_amazon_affiliate_code_feedback() {
	...
	if(!empty($amazon_affiliate_code)) {
		if(preg_match('/^\w+\-[0-9]+$/', $amazon_affiliate_code)) {
		...
		<tbody>
			<tr>
				...
				<td>
					...
					<p class="description">
						<?php _e('You can find your Amazon affiliate tracking ID by visiting your <a href="https://affiliate-program.amazon.com/gp/associates/network/main.html" target="_blank">Amazon Affiliate Homepage</a>. The code should be near the top left of the screen and will end in "-20" if you live in the United States of America. <a href="'.admin_url('admin.php?page=mbt_help&mbt_video_tutorial=amazon_affiliates').'" target="_blank">Learn how to sign up for Amazon Associates.</a>' , 'mybooktable'); ?>
					</p>
...
function mbtpro_is_genius_link($url) {
...
function mbtpro_amazon_buybutton_init() {
	...
	if(mbt_get_setting('disable_amazon_affiliates')) {
		...
		remove_action('wp_ajax_mbt_amazon_buybutton_preview', 'mbt_amazon_buybutton_preview');
		add_action('wp_ajax_mbt_amazon_buybutton_preview', 'mbtpro_amazon_buybutton_preview');
...
function mbtpro_amazon_buybutton_preview() {
	if(empty($_REQUEST['data'])) { die(); }
	$id = mbt_get_amazon_AISN($_REQUEST['data']);
	if(mbtpro_is_genius_link($_REQUEST['data'])) {
...
function mbtpro_filter_amazon_buybutton_data($data, $store) {
	if(($data['store'] == 'amazon' or $data['store'] == 'kindle') and !empty($data['url']) and !mbtpro_is_genius_link($data['url'])) {
...
function mbtpro_kindle_instant_preview_init() {
	add_filter('mbt_get_kindle_instant_preview_box', 'mbtpro_filter_kindle_instant_preview_box', 10, 2);
}
add_action('mbtpro_init', 'mbtpro_kindle_instant_preview_init');

function mbtpro_filter_kindle_instant_preview_box($output, $post_id) {
...
function mbtpro_bnn_buybutton_init() {
	...
	if(mbt_get_setting('disable_cj_affiliates')) {
		...
		remove_action('wp_ajax_mbt_bnn_buybutton_preview', 'mbt_bnn_buybutton_preview');
		add_action('wp_ajax_mbt_bnn_buybutton_preview', 'mbtpro_bnn_buybutton_preview');
...
function mbtpro_bnn_buybutton_preview() {
	if(empty($_REQUEST['data'])) { die(); }
	if(mbtpro_is_genius_link($_REQUEST['data'])) {
		echo('<span class="mbt_admin_message_success">'.__('Valid Genius Link', 'mybooktable').'</span>');
...
function mbtpro_filter_bnn_buybutton_data($data, $store) {
	if(($data['store'] == 'bnn' or $data['store'] == 'nook') and !empty($data['url']) and !mbtpro_is_genius_link($data['url'])) {
...
function mbtpro_filter_kobo_buybutton_data($data, $store) {
	if($data['store'] == 'kobo' and !empty($data['url']) and !mbtpro_is_genius_link($data['url'])) {
...
function mbtpro2_filter_audible_buybutton_data($data, $store) {
	if($data['store'] == 'audible' and !empty($data['url']) and !mbtpro_is_genius_link($data['url'])) {
