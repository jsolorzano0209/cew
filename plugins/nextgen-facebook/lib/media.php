	...
	class NgfbMedia {
		...
		public function get_attachment_image_src( $pid, $size_name = 'thumbnail', $check_dupes = true, $force_regen = false ) {
			...
			if ( isset( $img_meta['width'] ) && isset( $img_meta['height'] ) ) {
				...
				$media_lib = __( 'Media Library', 'nextgen-facebook' );
				$edit_url  = get_edit_post_link( $pid );
				$func_name = 'wp_get_attachment_metadata()';
				$regen_msg = sprintf( __( 'You may consider regenerating the thumbnails of all WordPress Media Library images using one of <a href="%s">several available plugins from WordPress.org</a>.', 'nextgen-facebook' ), 'https://wordpress.org/plugins/search/regenerate+thumbnails/' );
				...
				if ( isset( $img_meta['file'] ) ) {
					...
					if ( $this->p->notice->is_admin_pre_notices() ) { // Skip if notices already shown.
						...
						$this->p->notice->err( sprintf( __( 'Possible %1$s corruption detected &mdash; the full size image dimensions for <a href="%2$s">image ID %3$s</a> are missing from the image metadata returned by the WordPress %4$s function.', 'nextgen-facebook' ), $media_lib, $edit_url, $pid, $func_name ).' '.$regen_msg, true, $dismiss_key, WEEK_IN_SECONDS );
					}
					...
					if ( $this->p->notice->is_admin_pre_notices() ) { // Skip if notices already shown.
						...
						// translators: %1$s is "Media Library" (translated), %2$s is the image editing URL, %3$s is the image ID, %4$s is a WordPress function name.
						$this->p->notice->err( sprintf( __( 'Possible %1$s corruption detected &mdash; the full size image file path for <a href="%2$s">image ID %3$s</a> is missing from the image metadata returned by the WordPress %4$s function.', 'nextgen-facebook' ), $media_lib, $edit_url, $pid, $func_name ).' '.$regen_msg, true, $dismiss_key, WEEK_IN_SECONDS );
					}
				}
			}
		...
		public function add_og_video_from_url( array &$og_video, $url ) {
			 ...
			 */
			$curl_opts = array( 'CURLOPT_USERAGENT' => NGFB_PHP_CURL_USERAGENT_FACEBOOK );
