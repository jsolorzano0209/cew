...
if ( ! class_exists( 'NgfbRegister' ) ) {
	...
	class NgfbRegister {
		...
		private static function uninstall_plugin() {
			...
			if ( empty( $opts['plugin_preserve'] ) ) {
				...
				foreach ( get_users() as $user ) {
					if ( ! empty( $user-> ID ) ) {	// just in case
						...
						delete_user_option( $user->ID, NGFB_DISMISS_NAME, false );	// $global = false
						delete_user_option( $user->ID, NGFB_DISMISS_NAME, true );	// $global = true
			...
			$dbquery = 'SELECT option_name FROM '.$wpdb->options.' WHERE option_name LIKE \''.$prefix.'ngfb_%\';';
		...
		private static function check_required( $cf ) {
			...
			foreach ( array( 'wp', 'php' ) as $key ) {
				...
				deactivate_plugins( NGFB_PLUGINBASE, true );	// $silent is true
