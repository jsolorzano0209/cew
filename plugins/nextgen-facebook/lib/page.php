	...
	class NgfbPage {
			...
			if ( empty( $title_text ) ) {
				...
				if ( empty( $title_text ) ) {
					if ( $title_text = get_bloginfo( 'name', 'display' ) ) {
						...
						$title_text = __( 'No Title', 'nextgen-facebook' );	// just in case
					}
				}
			}
		...
		public function get_description( $textlen = 160, $trailing = '...', $mod = false, $read_cache = true,
			...
			if ( empty( $desc_text ) ) {
				...
				if ( $mod['is_post'] ) {
					...
					if ( SucomUtil::is_tag_page( $mod['id'] ) ) {
						if ( ! $desc_text = tag_description( $mod['id'] ) ) {
							...
							if ( ! empty( $term_obj->name ) ) {
								$desc_text = sprintf( __( 'Tagged with %s', 'nextgen-facebook' ), $term_obj->name );
							}
						}
					} elseif ( SucomUtil::is_category_page( $mod['id'] ) ) {
						if ( ! $desc_text = category_description( $mod['id'] ) ) {
							$desc_text = sprintf( __( '%s Category', 'nextgen-facebook' ), get_cat_name( $mod['id'] ) );
						}
						...
						if ( ! empty( $term_obj->description ) ) {
							...
							$desc_text = sprintf( __( '%s Archive', 'nextgen-facebook' ), $term_obj->name );
						}
					}
					...
					if ( ! empty( $user_obj->description ) ) {
						...
						$desc_text = sprintf( __( 'Authored by %s', 'nextgen-facebook' ), $user_obj->display_name );
					}
					...
					$desc_text = sprintf( __( 'Daily Archive for %s', 'nextgen-facebook' ), get_the_date() );

				} elseif ( is_month() ) {

					$desc_text = sprintf( __( 'Monthly Archive for %s', 'nextgen-facebook' ), get_the_date('F Y') );

				} elseif ( is_year() ) {

					$desc_text = sprintf( __( 'Yearly Archive for %s', 'nextgen-facebook' ), get_the_date('Y') );

				} elseif ( SucomUtil::is_archive_page() ) {	// just in case

					$desc_text = sprintf( __( 'Archive Page', 'nextgen-facebook' ) );
			...
			if ( empty( $desc_text ) ) {
				...
				if ( $mod['post_status'] === 'auto-draft' ) {
					...
					$desc_text = __( 'No Description', 'nextgen-facebook' );	// just in case
				}
			}
			...
			if ( $textlen > 0 ) {
				...
				if ( $this->p->debug->enabled ) {
					$this->p->debug->log( 'description strlen before limit length '.strlen( $desc_text ).' (limiting to '.$textlen.' chars)' );
				}
		...
		public function get_the_content( array $mod, $read_cache = true, $md_idx = '' ) {
			...
			if ( $filter_content ) {
				...
				$max_time = SucomUtil::get_const( 'NGFB_CONTENT_FILTERS_MAX_TIME', 0.75 );
				$content_text = $this->p->util->safe_apply_filters( array( 'the_content', $content_text ), $mod, $max_time );
