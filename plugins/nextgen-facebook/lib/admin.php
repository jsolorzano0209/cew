...
if ( ! class_exists( 'NgfbAdmin' ) ) {
	...
	class NgfbAdmin {
		...
		public function load_menu_objects( $menu_libs = array() ) {
			...
			foreach ( $this->p->cf['plugin'] as $ext => $info ) {
				...
				self::$pkg[$ext]['aop'] = ! empty( $this->p->options['plugin_'.$ext.'_tid'] ) && 
					$aop && $this->p->check->aop( $ext, true, NGFB_UNDEF_INT ) === NGFB_UNDEF_INT ? true : false;
		...
		public function external_plugin_data( $res, $action = null, $args = null ) {
			...
			// get the acronym for the config
		...
		public function load_setting_page() {
				...
				if ( empty( $_GET[ NGFB_NONCE_NAME ] ) ) {	// NGFB_NONCE_NAME is an md5() string
					...
					switch ( $action_name ) {
						...
						case 'clear_hidden_notices':
							...
							$user = get_userdata( $user_id );
							$user_name = $user->display_name;
							delete_user_option( $user_id, NGFB_DISMISS_NAME, false );	// $global = false
							delete_user_option( $user_id, NGFB_DISMISS_NAME, true );	// $global = true
			...
			$this->add_footer_hooks();	// add plugin name and version to settings page footer
		...
		public function required_notices() {
			...
			if ( true === $pdir && empty( $this->p->options['plugin_'.$this->p->lca.'_tid'] ) &&
