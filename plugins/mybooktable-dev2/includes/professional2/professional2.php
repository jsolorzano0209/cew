...
function mbtpro2_frontend_init() {
	add_action('wp_enqueue_scripts', 'mbtpro2_enqueue_frontend_resources');
}
add_action('mbtpro2_init', 'mbtpro2_frontend_init');

function mbtpro2_enqueue_frontend_resources() {
...
function mbtpro2_gridview_hover_box() {
	...
	$output .= apply_filters('mbtpro2_gridview_hover_buybuttons', mbt_get_buybuttons($post->ID, true, true));
...
function mbtpro2_notifyme_buybutton_editor($output, $data, $id, $store) {
	if($data['store'] == 'notifyme') {
		...
		if(empty($mailchimp_api_key)) {
			$output .= '<span class="mbt_admin_message_failure">'.__('You need to set up your MailChimp API Key in order to use this button.', 'mybooktable').'<a href="'.admin_url('admin.php?page=mbt_settings&mbt_current_tab=5').'" target="_blank">'.__('Click here to go to your settings.', 'mybooktable').'</a></span>';
		} else {
			...
			<tbody>
				<tr>
					...
					<td rowspan="2">
						...
						<div class="mbt_api_key_feedback mbt_feedback mbt_feedback_beside"></div>
					</td>
				</tr>
				<tr>
					<td>
				...
				<tr>
					<td colspan="3">
						<p class="description"><?php printf(__('Insert your AWS Access Key ID and AWS Secret Key in order to enable %sAmazon Reviews%s and the Amazon Bulk Book Importer.', 'mybooktable'), '<a href="'.admin_url('admin.php?page=mbt_settings&mbt_current_tab=3').'">', '</a>'); ?> <a href="https://docs.aws.amazon.com/AWSECommerceService/latest/DG/becomingDev.html" target="_blank"> <?php _e('Learn how to get your AWS Access Key ID and AWS Secret Key', 'mybooktable'); ?></a></p>
					</td>
				</tr>
			</tbody>
		</table>
...
function mbtpro2_amazon_reviews_init() {
	add_filter('mbt_reviews_types', 'mbtpro2_add_amazon_reviews_type');
}
add_action('mbtpro2_init', 'mbtpro2_amazon_reviews_init');

function mbtpro2_add_amazon_reviews_type($reviews) {
	$aws_accesskey = mbt_get_setting('aws_access_key_id');
	$aws_secretkey = mbt_get_setting('aws_secret_key');
	...
	$reviews['amazon'] = array(
		...
		'book-check' => 'mbtpro2_check_book_amazon_reviews',
		'disabled' => $disabled,
	);
	return $reviews;
}
	...
	if(!empty($identifier)) {
		...
		if(!empty($response)) {
			...
			if(!empty($xml->Items->Item[0]->CustomerReviews->IFrameURL)) {
				...
				$output .= '<iframe id="mbt_amazon_reviews" src="'.$url.'" frameborder="0"></iframe>';
			}
		}
	}
...
function mbtpro2_check_book_amazon_reviews($book_id) {
	$isbn = get_post_meta($book_id, 'mbt_unique_id_isbn', true);
	$asin = get_post_meta($book_id, 'mbt_unique_id_asin', true);
	if(empty($isbn) and empty($asin)) {
		return '<span class="mbt_admin_message_failure">'.__('No ISBN or ASIN entered.', 'mybooktable').'</span>';
	} else {
		if(!empty($isbn)) {
			$matches = array();
		...
		if(!empty($asin)) {
			...
			if(!empty($matches[1])) {
				return '<span class="mbt_admin_message_success">'.__('Valid ASIN', 'mybooktable').' <a href="http://www.amazon.com/dp/'.$asin.'" target="_blank">'.__('(verify book)', 'mybooktable').'</a></span>';
			}
		}
		return '<span class="mbt_admin_message_failure">'.__('Invalid ISBN and ASIN', 'mybooktable').'</span>';
	}
}
...
function mbtpro2_amazon_import_init() {
	add_filter('mbt_importers', 'mbtpro2_add_amazon_importer');
	add_filter('mbt_pre_import_book', 'mbtpro2_amazon_books_import_filter_book', 10, 2);
}
add_action('mbtpro2_init', 'mbtpro2_amazon_import_init');

function mbtpro2_add_amazon_importer($importers) {
	$importers['amazon'] = array(
		...
		'get_book_list' => array(
			'render_import_form' => 'mbtpro2_render_amazon_books_import_form',
			'parse_import_form' => 'mbtpro2_parse_amazon_books_import_form',
		),
	);
	return $importers;
}

function mbtpro2_render_amazon_books_import_form() {
	$aws_accesskey = mbt_get_setting('aws_access_key_id');
	$aws_secretkey = mbt_get_setting('aws_secret_key');
...
function mbtpro2_parse_amazon_books_import_form() {
...
function mbtpro2_amazon_books_import_get_asin($identifier) {
	$matches = array();
...
function mbtpro2_amazon_books_import_get_isbn($identifier) {
	$matches = array();
...
function mbtpro2_amazon_books_import_filter_book($data, $import_type) {
	if($import_type !== 'amazon') { return $data; }

	$identifier_parse = $data['format'] == 'asin' ? 'mbtpro2_amazon_books_import_get_asin' : 'mbtpro2_amazon_books_import_get_isbn';
	...
	if(empty($book['unique_id_isbn']) and $data['format'] == 'isbn') { $book['unique_id_isbn'] = $identifier; }
	if(!empty($item->ASIN)) { $book['unique_id_asin'] = (string)$item->ASIN; }
	if(empty($book['unique_id_asin']) and $data['format'] == 'asin') { $book['unique_id_asin'] = $identifier; }
