...
function mbt_pop_query($name) {
	...
	if($name !== $query['name']) {
		trigger_error('MyBookTable tried to close '.$name.' query but was prevented by '.$query['name'].' query!', E_USER_WARNING);
...
function mbt_send_tracking_data() {
	...
	$book_metas = array_map(function($post) { return get_post_custom($post->ID); }, $books);
