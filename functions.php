...
// extraaaaaaaaaaaaaaaaaaaaaa
include(get_stylesheet_directory() . '/custom_slider.php');
add_shortcode('custom_sc', 'some_random_code');  

//extraaaaaaaaaaaaaaaaaaaaa
...
add_action( 'wp_footer', 'cf7_js_conversions' );
 
function cf7_js_conversions() {
?>

<script type="text/javascript">
document.addEventListener( 'wpcf7mailsent', function( event ) {
    if ( '1248' == event.detail.contactFormId ) {
        ga('send', 'event', {eventCategory: 'Contact',eventAction: 'College Preference Survey',eventLabel: 'Contact Form',eventValue: '10'});
    }
    if ( '1249' == event.detail.contactFormId ) {
        ga('send', 'event', {eventCategory: 'Get Started',eventAction: 'Lead',eventLabel: 'Contact Form',eventValue: '25'});
        fbq('track','Lead');
    }
    if ( '1278' == event.detail.contactFormId ) {
        ga('send', 'event', {eventCategory: 'Contact',eventAction: 'Lead',eventLabel: 'Contact Form',eventValue: '25'});
        fbq('track','Lead');
    }
    if ( '133' == event.detail.contactFormId ) {
        ga('send', 'event', {eventCategory: 'Newsletter',eventAction: 'Newsletter Sign Up',eventLabel: 'Newsletter Sign Up Form',eventValue: '10'});
    	fbq('track','Lead');
    }
}, false );
</script>
...
add_filter( 'genesis_post_info', 'cew_post_info_filter', 12 );
function cew_post_info_filter($post_info) {
	$author_op = postauthors_html();
	$post_info = "[post_date] by {$author_op} [post_comments] [post_edit]";
	return $post_info;
}

function postauthors_html(){
